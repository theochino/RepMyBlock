<?php 
	...
	if (! empty ($RawVoterID) ) {
		$p = new OutragedDems();
		$resultVoter = $p->SearchVoterDBbyID($DatedFiles, $RawVoterID);
	...
	<BODY>
		...
		<h1>Election District <?= $ElectionDistrict ?>, Assembly District <?= $AssemblyDistrict ?></H1>
			  ...
			  <div class="column" style="background-color:#aaa;">
			  	 <h2>Democrat Electors: 
			    <?= $NumberOfElectors ?></h2>
			    <h2>Required Signatures:
			    <?= $NumberOfSignatures ?></H2>
			    <h2>Number of Buildings: <?= $NumberOfAddressesOnDistrict ?></h2>
			    
			  </div>
			  <div class="column" style="background-color:#fff;">
			  	<P><a href="commited/?<?= $URLAction ?>" class="btn success">I want to run</a></P><BR>
			  	<P><a href="thinkabout.php?<?= $URLAction ?>" class="btn info">I'll think about it</a></P><BR>
